...
local function checkAABB( l ,t, w,h)
 ...
 local function adjustPosition(self)
  ...
  local function adjustScale(self)
    ...
    local rw , rh = getVisibleArea(self ,1 )
    local sx ,sy = rw /ww ,rh/wh
    local rscale  =max(sx ,sy)
    self.scale  = max(self.scale ,rescale)
  end
...
function gamera.new(l ,t, w ,h)
   ...
   x = 0 , y = 0 ,
   scale=1 ,angle=0 ,sin= math.sin(0), cos= math.cos(0),
   l = 0 ,t= 0 ,w=sw ,h =sh , w2 = sw* 0.5 ,h2 = sh*0.5}, gameraMt)
